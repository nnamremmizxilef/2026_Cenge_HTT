...
# --- Add 0.1% quantile per node (computed from ks_valid) ----------------------
ks_q0001 <- ks_valid %>%
  group_by(node) %>%
  summarise(
    ks_0.1_quantile = as.numeric(quantile(ks, probs = 0.001, na.rm = TRUE)),
    .groups = "drop"
  )

node_summary <- node_summary %>%
  left_join(ks_q0001, by = "node")
...
node_summary_long <- node_summary %>%
  select(node, ks_min, ks_0.1_quantile, ks_0.5_quantile, ks_5_quantile, ks_median, ks_mean) %>%
  pivot_longer(cols = -node, names_to = "statistic", values_to = "value") %>%
  mutate(
    statistic = factor(
      statistic,
      levels = c("ks_min", "ks_0.1_quantile", "ks_0.5_quantile", "ks_5_quantile", "ks_median", "ks_mean"),
      labels = c("Min", "0.1% quantile", "0.5% quantile", "5% quantile", "Median", "Mean")
    )
  )
...
p7 <- ggplot(node_summary_long, aes(x = node, y = value, color = statistic, group = statistic)) +
  ...
  labs(x = "Node", y = "Ks (log scale)", color = "Statistic") +
  theme_pubr() +
  theme(
    axis.text.x = element_text(angle = 45, hjust = 1, size = 8),
    axis.text.y = element_text(size = 9),
